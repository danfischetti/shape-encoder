...
function init()
  ...
  model.ds = {}
  model.start_s = {}
  model.h_err = torch.Tensor(params.batch_size,72,8,8):cuda()
  for j = 0, params.n_glimpses do
    model.s[j] = {}
    for d = 1, params.rnn_layers do
      model.s[j][d] = torch.zeros(params.batch_size,72,8,8):cuda()
    end
  end
  for d = 1,params.rnn_layers do
    model.start_s[d] = torch.zeros(params.batch_size,72,8,8):cuda()
    model.ds[d] = torch.zeros(params.batch_size,72,8,8):cuda()
  end
	model.n = 1344
...
local function bp()
  ...
  local rec_err = model.criterion:backward(rec,model.x_in[params.n_glimpses])*aux_factor
  local x_dec_err = model.decoder:backward(x_enc,rec_err)
  x_enc_err:add(x_dec_err)
  ...
  for i = params.n_glimpses-1, 1, -1 do
    ...
    rec_err = model.criterion:backward(rec,model.x_in[i])*aux_factor
...
function main()
  ...
  while loop do
    ...
    train_err = train_err/(#model.targets*model.train_batches*16)
    valid_err = valid_err/(#model.targets*model.valid_batches*16)
